-- =====================================================================
-- COMPREHENSIVE MIGRATION: Remove Grade 12 + Fix Bulk Upload
-- Run this in your Supabase SQL Editor
-- =====================================================================

-- ═══════════════════════════════════════════════════════════════════════
-- PART 1: Fix Bulk Upload (UPSERT with grade_level and section support)
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
-- PART 2: Remove All Grade 12 Data
-- ═══════════════════════════════════════════════════════════════════════

-- Step 1: Delete votes cast by Grade 12 voters
DELETE FROM votes
WHERE voter_id IN (
  SELECT p.user_id
  FROM profiles p
...
-- Step 2: Delete votes for Grade 12 candidates
DELETE FROM votes
WHERE candidate_id IN (
  SELECT id FROM candidates WHERE grade_level = 'Grade 12'
);

-- Step 3: Delete Grade 12 candidates
DELETE FROM candidates
WHERE grade_level = 'Grade 12';

-- Step 4: Delete user_roles for Grade 12 voters
...
-- Step 5: Delete Grade 12 voter profiles
DELETE FROM profiles
WHERE grade_level = 'Grade 12';

-- Step 6: Delete Grade 12 users (voters only, not admin)
DELETE FROM users
WHERE id IN (
...
-- Step 7: Delete the "Grade 12 Representative" position
DELETE FROM positions WHERE lower(title) = 'grade 12 representative';

-- Step 8: Clean up archive tables (if they exist)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'election_results_archive') THEN
    DELETE FROM election_results_archive WHERE candidate_grade = 'Grade 12';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'election_voter_groups_archive') THEN
    DELETE FROM election_voter_groups_archive WHERE voter_grade = 'Grade 12';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'election_results_voter_breakdown') THEN
    DELETE FROM election_results_voter_breakdown WHERE voter_grade = 'Grade 12';
  END IF;
END $$;

-- ═══════════════════════════════════════════════════════════════════════
-- PART 3: Update functions that reference Grade 12 representative logic
-- ═══════════════════════════════════════════════════════════════════════

-- Update app_add_voter (ensure it works properly)
...
-- Update app_list_voters with LEFT JOIN to safeguard missing profiles
...
SELECT 
  ...
  (SELECT COUNT(*) FROM positions WHERE lower(title) LIKE '%grade 12%') as remaining_grade12_positions;
